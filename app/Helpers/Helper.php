...
{
    if ($status == 'Active') {
        return '<span class="badge light badge-success">' . $status . '</span>';

    } elseif ($status == 'Inactive') {
        return '<span class="badge light badge-warning text-dark">' . $status . '</span>';

    } elseif ($status == 'Deleted') {
        return '<span class="badge light badge-danger">' . $status . '</span>';

    } elseif (in_array($status ,['Pending','Late'] )) {
        return '<span class="badge light badge-warning">' . $status . '</span>';

    } elseif (in_array($status, ['Approved', 'On-time'])) {
        return '<span class="badge light badge-success">' . $status . '</span>';
    ...
    } elseif ($status == 'Approve & On Process') {
        return '<span class="badge light badge-success">' . $status . '</span>';
        ...
        return '<span class="badge text-dark light badge-secondary">' . $status . '</span>';
    }
}
...
function getLinkLabel($linkText = null, $icon = null, $class = null)
{
    ...
    if (is_null($linkText) && is_null($icon)) {
        return '';
    }

    return '<span title="' . ($linkText ?? '') . '" class="' . ($class ?? '') . '">'
        . ($icon ? $icon : '')
        . ' '
        . ($linkText ? $linkText : '')
        . '</span>';
...
function banglaDate($englishDate, $expectedFormat)
{

    $dateTime = new DateTime($englishDate);

    $dateFormatter = new IntlDateFormatter('bn_BD', IntlDateFormatter::FULL, IntlDateFormatter::FULL, 'Asia/Dhaka', IntlDateFormatter::TRADITIONAL);

    $dateFormatter->setPattern($expectedFormat);

    return $dateFormatter->format($dateTime);
}

function sms_send($mobile, $sms)
{
    $url = env('SMS_API_URL');
...
function getAttendanceStatus($checkIn, $checkOut, $attendanceStatus)
{

    if (strtolower($attendanceStatus) === 'absent') {
        return 'Absent';
    }

    $companyEntryTime = '09:00:00';
    $companyExitTime = '17:00:00';

    $checkInTime = time24HoursFormat($checkIn);
    $checkOutTime = time24HoursFormat($checkOut);

  
    $companyEntryTimestamp = strtotime($companyEntryTime);
    $checkInTimestamp = strtotime($checkInTime);

    if ($checkInTimestamp > $companyEntryTimestamp + (20 * 60)) {
        return 'Late';
    }
