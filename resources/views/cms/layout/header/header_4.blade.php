<div class="bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 text-white sticky top-0 z-50 shadow-lg">
  <div class="container mx-auto px-4">
    <div class="flex justify-between items-center h-20">
      <!-- Logo -->
      <div class="flex-shrink-0">
        <h1 class="font-bold text-2xl text-white hover:text-gray-100 transition-all duration-300 transform hover:scale-105">BrandName</h1>
      </div>
        ...
        @foreach($website_menus as $menu)
          <div class="relative group">
            <a href="/{{ $menu->page?->slug }}"
               class="px-4 py-2 text-white hover:text-gray-100 font-medium text-sm uppercase tracking-wide transition-all duration-300 flex items-center">
              {{ $menu->name }}
                ...
                <div class="py-2">
                  @foreach($menu->children as $child)
                    <a href="/{{ $child->page?->slug }}"
                       class="block px-4 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 text-sm">
                      {{ $child->name }}
                    </a>
                  @endforeach
                </div>
