<div class="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 text-white sticky top-0 z-50">
  <div class="container mx-auto px-4">
    <div class="flex justify-between items-center h-20">
      <!-- Logo -->
      <div class="flex-shrink-0">
        <h1 class="font-bold text-2xl text-white hover:text-gray-200 transition-colors duration-300">BrandName</h1>
      </div>
        ...
        @foreach($website_menus as $menu)
          <div class="relative group">
            <a href="/{{ $menu->page?->slug }}"
               class="px-4 py-2 text-white hover:text-gray-200 font-medium text-sm uppercase tracking-wide transition-all duration-300 flex items-center">
              {{ $menu->name }}
                ...
                <div class="py-2">
                  @foreach($menu->children as $child)
                    <a href="/{{ $child->page?->slug }}"
                       class="block px-4 py-3 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors duration-200 text-sm">
                      {{ $child->name }}
                    </a>
                  @endforeach
                </div>
  ...
  <div class="md:hidden hidden bg-white/10 backdrop-blur-sm border-t border-white/20" id="mobile-menu">
    <div class="container mx-auto px-4 py-2 space-y-1">
      @foreach($website_menus as $menu)
        <div class="py-2">
          <button type="button"
                  class="w-full flex items-center justify-between text-white hover:text-gray-200 font-medium transition-colors duration-300"
          ...
          @if($menu->children->count() > 0)
            <div class="hidden pl-4 mt-2 space-y-2 submenu">
              @foreach($menu->children as $child)
                <a href="/{{ $child->page?->slug }}"
                   class="block text-gray-200 hover:text-white transition-colors duration-300">
                  {{ $child->name }}
                </a>
              @endforeach
            </div>
