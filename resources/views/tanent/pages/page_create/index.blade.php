...
@section('content')
    <div class="max-w-6xl mx-auto py-6 px-2 sm:px-4">
        <div class="bg-white shadow-xl rounded-2xl p-4 sm:p-8">
            ...
            @endif
            <form method="POST" action="{{ isset($page) ? route('tanent.website.page_update', [ $page->id]) : route('tanent.website.page_store') }}" id="pageForm" class="space-y-6 sm:space-y-8">
                @csrf
                @if (isset($page))
                    @method('PUT')
                @endif
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#openModalBtn').on('click', function() {
                ...
                // আগের selected component গুলো checked করা
                $('.component-checkbox').each(function() {
                    var name = $(this).val();
                    $(this).prop('checked', selectedComponents.includes(name));
                });
            });
            ...
            // If editing, initialize selected components
            @if(isset($page) && $page->components)
                var initialComponents = @json(is_string($page->components) ? json_decode($page->components, true) : $page->components);
                if (Array.isArray(initialComponents)) {
                    componentPositions = initialComponents.map(function(c, i) {
                        return {
                            name: c.name,
                            position: i + 1
                        };
                    });
                    selectedComponents = componentPositions.map(function(c) { return c.name; });
                    updateSelectedComponentsUI();
                }
            @endif
        });
